...
export default function UpdateForm() {
    ...
    const defaultValues = useMemo(
        () => ({
            RawNewAutoQuotes: reports?.RawNewAutoQuotes || '',
            RawNewAutoWritten: reports?.RawNewAutoWritten || '',
            TotalFireWritten: reports?.TotalFireWritten || '',
            LifeApplications: reports?.LifeApplications || '',
            AnnualizedLifePremium: reports?.AnnualizedLifePremium || '',
            HealthApplications: reports?.HealthApplications || '',
            AnnualizedHealthPremium: reports?.AnnualizedHealthPremium || '',
            OtherFinancialServices: reports?.OtherFinancialServices || '',
        }),
    ...
    const onSubmit = async (data) => {
        try {
            const RepUpID = localStorage.getItem('RepUpID');
            ...
            const response = await axios.post(`api/seller/report/update/${RepUpID}`, formData);
            const { message } = response.data;
            enqueueSnackbar(message);
            localStorage.removeItem('RepUpID');
